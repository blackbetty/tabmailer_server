...
// function pickUserLinksToSend(users) {
//     var usersWithArticles = _.reject(users, (user) => { user.article_list.length === 0 });
//     // sync array processing, no need for async
// }
...
function filterUsersForDeliveryPreferenceMatch(users) {
    ...
    var dailyRecipients = _.filter(users, (user) => { return user.settings.frequency === 'DAILY' });

    var weeklyRecipientsForToday = _.filter(users, (user) => { return (user.settings.frequency === 'WEEKLY' && user.settings.frequency_day == moment().day()) });

    var monthlyRecipientsForToday = _.filter(users, (user) => { return (user.settings.frequency === 'MONTHLY' && user.settings.frequency_day == moment().day()) });
...
function createDailyRandomLinkCollectionObjectCollection(users) {
    ...
    // LUL
    linkCollectionObjectCollection = [];
